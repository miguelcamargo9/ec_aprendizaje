...
class ClientsController extends Controller {
      ...
      try {
        ...
        foreach ($clientchildren as $client) {
          ...
          if (is_numeric($found_key)) {
            $burndate = substr($children[$found_key]['burndate'], 0, 10);
            $burntime = (isset($children[$found_key]['burntime'])) ? substr($children[$found_key]['burntime'], 11, 8) : null;
            Child::where('id', '=', $client->id_hijo)->update(
                    array(
                        'nombre' => $children[$found_key]['name'],
                        'colegio' => $children[$found_key]['school'],
                        'calendario' => $children[$found_key]['calendar'],
                        'curso' => $children[$found_key]['course'],
                        'fecha_nacimiento' => $burndate,
                        'hora_nacimiento' => $burntime,
                        'edad' => $children[$found_key]['age'],
        ...
        foreach ($children as $child) {
          if (!isset($child['id'])) {
            ...
            $burntime = (isset($child['burntime'])) ? substr($child['burntime'], 11, 8) : null;
            $id_child = $this->insertChild($child['name'], $child['school'], $child['calendar'], $child['course'], $burndate, $burntime, $child['age'], $idUser, $idUserTwo);
            $this->insertClient($idUser, $id_child);
          }
        }

        return response()->json(array('success' => true, 'msj' => 'Cliente Editado con Éxito'));
      } catch (Exception $ex) {
        return response()->json(array('error' => array('Error editanto el Cliente')));
      }
    } else {
    ...
    if ($validator->passes()) {
      try {
        ...
        foreach ($children as $key => $child) {
          $burndate = substr($child['burndate'], 0, 10);
          $burntime = (isset($child['burntime'])) ? substr($child['burntime'], 11, 8) : null;
          $id_child = $this->insertChild($child['name'], $child['school'], $child['calendar'], $child['course'], $burndate, $burntime, $child['age'], $id_user, $id_user_two);
  ...
  public function insertChild($name, $school, $calendar, $course, $burndate, $burntime, $age, $id_user, $id_user_two) {
    $newchild = new Child;
    ...
    $newchild->hora_nacimiento = $burntime;
    $newchild->edad = $age;
